...
import dramsim_format_convert_trace as dramsim_format
...
def gen_bw_numbers(dram_ifmap_trace_file, dram_filter_trace_file,
    ...
    log = str(dram_activation_bw) + ",\t" + str(dram_filter_bw) + ",\t" + str(dram_ofmap_bw) + ",\t" + str(
        sram_read_bw) + ",\t" + str(sram_ofmap_bw) + ","
...
def test_dram_trace_read_dramsim(ifverbose=True):
    os.system("pwd")
    os.system("rm *.csv")
    test_fc1_24x24 = [27, 37, 512, 27, 37, 512, 1, 24, 1]
    param = test_fc1_24x24
    ...
    # generate the ws dataflow sram traces
    sram_cycles, util = \
        sram_ws.sram_traffic(
            ...
            filt_h=filt_h, filt_w=filt_w,
            num_channels=num_channels,
            strides=strides, num_filt=num_filters,
            ofmap_base=ofmap_base, filt_base=filter_base, ifmap_base=ifmap_base,
            sram_read_trace_file=sram_read_trace,
            sram_write_trace_file=sram_write_trace
        )

    dramsim_format.dram_trace_read_dramsim(
        sram_sz=ifmap_sram_size,  # test for the dram part corresponding to ifmap offset/base sram
        word_sz_bytes=word_sz,
        min_addr=ifmap_base, max_addr=filter_base,
        sram_trace_file=sram_read_trace,
        dram_trace_file=dram_ifmap_read_trace,
        verbose=ifverbose
    )

    dramsim_format.dram_trace_read_dramsim(
        sram_sz=filter_sram_size,  # test for the dram part corresponding to ifmap offset/base sram
        word_sz_bytes=word_sz,
        min_addr=filter_base, max_addr=ofmap_base,
        sram_trace_file=sram_read_trace,
        dram_trace_file=dram_filter_read_trace,
        verbose=ifverbose
    )
...
def cleanup():
    ...
    net_name = 'test_dramsim'

    # path = "./output/scale_out"
    path = "./outputs/" + net_name + "_" + 'ws'
